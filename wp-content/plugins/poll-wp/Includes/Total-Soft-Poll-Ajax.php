<?php
	...
	function T_S_P_Vimeo_Video_Image_Callback()
	{
		if(empty($_POST['nonce']) || !wp_verify_nonce( $_POST['nonce'], 'nonce-for-polll' )){
			...
			$TSoft_Poll_Image_Src_Real = wp_remote_get("http://vimeo.com/api/v2/video/$tsp_image_src[1].php");
			$TSoft_Poll_Image_Src_Real = unserialize(wp_remote_retrieve_body( $tsp_image_res ));
	...
	{
		$newNonce = wp_create_nonce('nonce-for-polll');
		if( !wp_verify_nonce($_POST['nonce'],'nonce-for-polll')){
			echo $_POST['nonce'];
			wp_die("");
		}
